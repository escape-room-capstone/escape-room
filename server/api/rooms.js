...
//HERE IS WHERE WE ARE TRYING TO UPDATE OUR PUZZLES SO USER HAS FREEDOM TO MOVE PUZZLES TO ANY ROOM
router.put('/', async (req, res, next) => {
  try {
    ...
    const puzzleNames = Object.values(req.body).reduce((roomsObj, currentRoom, idx) => {
        roomsObj[roomIdArray[idx]] = currentRoom.puzzles
        return roomsObj
    }, {})        
//console logging our puzzleNames object, which is an object with the keys being the roomID, and within puzzleNames we have another object at puzzleNames.puzzles where our keys are puzzleID
    console.log("Puzzle OBJECT", puzzleNames);
    

    for(let i = 0; i < roomIdArray.length; i++){
      let currentRoomId = roomIdArray[i];      
      let currentRoom = (await Room.findByPk(currentRoomId))
      let objectLength = Object.keys(puzzleNames[currentRoomId]).length
      for(let i = 0; i < objectLength; i++){
        let currentPuzzleId = Object.keys(puzzleNames[currentRoomId])[i] * 1;
        let currentPuzzleName = Object.values(puzzleNames[currentRoomId])[i];

        //slicing off the number from currentPuzzleName to get the ID for that puzzle 
        let tempId = currentPuzzleName.split("Puzzle").join("") * 1;
        console.log(tempId, "TEMP ID");        

        //finding line item by currentPuzzleId (which is the key in our object)
        const lineItem = (await RoomData.findOne({ where : { roomId : currentRoomId, puzzleId : currentPuzzleId*1}}))
        console.log(await lineItem.dataValues, "before update");
        
        //trying to update our lineItems puzzleId to be the tempId (id which we generated by slicing off the number off of our puzzleName)
        await lineItem.update({ puzzleId : tempId })        


        //returns the same exact lineItem from line 50...
        console.log(await lineItem.dataValues, "after update");
      }                        

      currentRoom.number = roomNumberArr[i].roomNumber;
