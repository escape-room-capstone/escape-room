...
//There are some numbers in the room written in a board, thinking of adding another puzzle where the answer will be all of those numbers added up
  ...
  return (
    <div className="cashDesk" onClick={handleClickedGun}>
        ...
        <p>Looks like we made it inside the bank... You may have to canvas the area for a weapon.</p>
      ...
      <Modal isOpen={showWeapon}>
          ...
          <button onClick={() => props.history.push("/Bankgame3")}> Proceed </button>
          <button style={{ marginLeft:"10px"}} onClick={() => setShowWeapon(false)}>Keep Searching Room</button>
      </Modal>
        <button style={{ float:"right" }} onClick={handleHint}>Click for a hint</button>
        <div className="gun">
            <img style={{  marginTop: "50px", marginLeft: "100px", height:"4px", width:"4px"}} src={Gun} alt=""></img>
        </div>
