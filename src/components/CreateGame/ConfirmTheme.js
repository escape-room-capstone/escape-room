...
const ConfirmTheme = (props) => {
  ...
  return (
    <div>         
        ...
        {theme.themeImages.map((image, idx) => {
            return (
              <img style={{ width: "48vw", margin: "10px" }} key={idx} src={image} alt=""></img>
            )
          })}
          <div style={{ position:"fixed", left:"46%", top:"50%"}}>
          <button onClick={()=>props.history.push("/createtheme")}> Back </button>
          <button  onClick={()=> props.history.push(`/creategame/${theme.id}`)} style={{marginLeft:"10px"}}> Use This Theme </button>
          </div>
    </div>
  );
};
